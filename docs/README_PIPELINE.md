...
- `review_fts` là BM25; chỉ mục E5 riêng và hybrid RRF chạy sau `src/research.py build-dense`.

## Chuẩn bị

Chạy `scripts/windows/setup_web.bat`, nhập key và chạy `scripts/windows/train_model.bat`. Các lệnh dưới đây dùng
`.venv\Scripts\python.exe` và database `instance\food_reviews.sqlite3`.

Kiểm tra trạng thái:

```bat
.venv\Scripts\python.exe src/data_pipeline.py status
.venv\Scripts\python.exe src/research.py serp-quota
```
...
```bat
.venv\Scripts\python.exe src/data_pipeline.py seed --area "Hải Châu, Đà Nẵng" --cuisine "món Việt"
```

Tìm và upsert tối đa 20 nhà hàng:

```bat
.venv\Scripts\python.exe src/data_pipeline.py ingest-restaurants --area "Hải Châu, Đà Nẵng" --cuisine "món Việt" --limit 20
```

Chạy lần lượt các seed đang bật và tự dừng khi API báo hết ngân sách/hạn mức:

```bat
.venv\Scripts\python.exe src/data_pipeline.py ingest-seeds --max-seeds 5 --per-seed-limit 20 --daily-limit 25
```

Lấy review cho tối đa 20 nhà hàng, mỗi nhà hàng tối đa một trang trong lần chạy:

```bat
.venv\Scripts\python.exe src/data_pipeline.py ingest-reviews --max-restaurants 20 --pages 1
```
...
```bat
.venv\Scripts\python.exe src/data_pipeline.py ingest-reviews --max-restaurants 20 --pages 1 --refresh
```
...
```bat
.venv\Scripts\python.exe src/data_pipeline.py analyze-pending --limit 1000
```

Tạo baseline thống kê rating và polarity theo tháng:

```bat
.venv\Scripts\python.exe src/data_pipeline.py compute-trends
```
...
.venv\Scripts\python.exe src/data_pipeline.py search-index "món ngon phục vụ tốt" --limit 5
```

Sau `build-index`, chạy `.venv\Scripts\python.exe src/research.py build-dense` để tạo
...
lượt tìm kiếm mới; cần cài `requirements/requirements_research.txt` và tải model embedding.

Đóng dấu một phiên bản dữ liệu để sử dụng trong thí nghiệm:

```bat
.venv\Scripts\python.exe src/data_pipeline.py snapshot --notes "Dataset trước thí nghiệm ABSA v1"
```

Xuất data card và báo cáo chất lượng:

```bat
.venv\Scripts\python.exe src/data_pipeline.py data-report
```
...
```bat
.venv\Scripts\python.exe src/data_pipeline.py evaluate-retrieval --queries evaluation_queries.json --k 5
.venv\Scripts\python.exe src/research.py evaluate-retrieval --queries evaluation_queries.json --split test
```
...
[`docs/research/TRAINING_IMPROVEMENT_PLAN.md`](research/TRAINING_IMPROVEMENT_PLAN.md).
