...
def retrieve(store,query,restaurant_ids=None,limit=8,search_id=None,log=True):
    ...
    sql+=' ORDER BY raw_score LIMIT ?';params.append(limit)
...
def evaluate(store,query_file,k=5,output_path=None):
    ...
    for row in rows:
        ...
        result=retrieve(store,row['query'],limit=max(k,10),log=False)
        ...
        details.append({'id':row['id'],'split':row.get('split'),'query':row['query'],
            ...
            'retrieved_review_ids':review_rank[:k],'retrieved_restaurant_ids':restaurant_rank[:k]})
    def aggregate(key):
    ...
    report={'method':'SQLite FTS5 BM25','k':k,'queries_in_file':len(rows),
